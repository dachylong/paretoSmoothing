...
getPropensityScoreInCurrentEnvironment <- function(x, z, p.score = "logistic"){
  ...
  if (p.score == "logistic") {
    ...
    formulaString <- paste0("z ~ ", paste0(mainEffects, collapse = " + "), " + ", paste0(quadEffects, collapse = " + "))
...
generateDataForIterInCurrentEnvironment <- function(iter, x, z, w, overlap = TRUE, covariates = "select", setting = "A", p.score = "none") {
  ...
  if (setting == "B" || setting == "C") {
    ...
    formulaString <- paste0("y ~ ",paste0(mainEffects, collapse = " + "), " + ",paste0(quadEffects, collapse = " + "))
